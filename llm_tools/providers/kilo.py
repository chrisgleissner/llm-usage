...
def _parse_kilo_stats_text(text: str) -> dict[str, Any] | None:
    ...
    Looks for ``key: value`` lines and ``key = value`` lines for the small
    set of fields we care about. A trailing currency/unit token (e.g.
    ``balance: 7.50 GBP``) is folded into the parsed value. Anything
    ...
    for line in text.splitlines():
        ...
        for regex, keys in patterns:
            ...
            if value_key in ("balance", "budget", "spent"):
                ...
                out[value_key] = value
            if extra_key and match.group(2):
